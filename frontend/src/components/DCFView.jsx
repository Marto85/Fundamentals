    ...
    <div className="relative inline-block ml-2">
      ...
      {open && (
        <>
          ...
          <div className="absolute z-50 bottom-full mb-2 w-64 -left-28 sm:left-1/2 sm:-translate-x-1/2 bg-card border border-gold/30 p-4 rounded-xl shadow-2xl text-xs text-subtle text-left font-body fade-in cursor-default" onClick={(e) => e.stopPropagation()}>
...
export default function DCFView() {
  ...
  const loadCompany = useCallback(async (symbol) => {
    setLoading(true); setError(null); setData(null); setResult(null); setGrowthRate('')
    try {
  ...
  useEffect(() => { if (!ticker) { setData(null); setResult(null); setGrowthRate('') } }, [ticker])

  const calculateDCF = (fcf, netDebt, shares, g, tg, r) => {
    if (r <= tg) return null
    let pvSum = 0, currentFcf = fcf, projectedFlows = []
    for (let i = 1; i <= 5; i++) {
      currentFcf *= (1 + g)
  ...
  const handleCalculate = () => {
    ...
    if (isNaN(g) || isNaN(tg) || isNaN(r)) return alert("Valores numéricos inválidos.")
    if (r <= tg) return alert("El WACC debe ser mayor a la Tasa Terminal.")
    ...
    const sensitivityMatrix = waccSteps.map(wDelta => {
      ...
      return {
        ...
        row: growthSteps.map(gDelta => {
          ...
          return { g: testG, val: calc ? calc.intrinsicValue : null }
        })
      }
    })

    setResult({ ...baseCalc, marginOfSafety, currentPrice, sensitivityMatrix, baseG: g, baseR: r })
  }
    ...
    if (!fcf || fcf <= 0 || !shares) {
      return (
        <div className="glass rounded-2xl p-8 text-center mt-8 border border-border/50 fade-in">
          ...
          <h3 className="font-display font-semibold text-text mb-2">Datos insuficientes</h3>
          <p className="text-subtle text-sm max-w-md mx-auto">La empresa no reporta Flujo Libre de Caja positivo o faltan datos de acciones.</p>
        </div>
      )
    }
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="glass rounded-2xl p-6 border border-border/50">
            <h3 className="flex items-center gap-2 text-sm font-display uppercase tracking-widest text-muted mb-6">
              <FileText size={16} /> 1. Datos del Balance
            </h3>
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              <div>
                ...
                <p className="text-[10px] text-muted mt-2 border-t border-border/50 pt-1.5 leading-tight">
                  ...
                  {fcfCagr !== null && fcfCagr !== undefined ? <><span className="text-emerald font-mono font-medium">{fmtPct(fcfCagr)}</span> (FCF)</> : 
                   (niCagr !== null && niCagr !== undefined ? <><span className="text-sky font-mono font-medium">{fmtPct(niCagr)}</span> (Beneficio Neto)</> : 
              ...
              <div>
                <div className="flex items-center mb-2">
                  ...
                  <PopoverInfo title="Tasa a Perpetuidad" content={<><p>Ritmo al que crecerá para siempre después del año 5.</p><p className="mt-2 text-gold"><strong>Estándar: 2% a 3%</strong></p></>} />
                </div>
                <div className="relative">
              ...
              <div>
                <div className="flex items-center mb-2">
                  ...
                  <PopoverInfo title="Costo del Capital" content={<><p>Rendimiento mínimo exigido para asumir el riesgo.</p><p className="mt-2 text-gold"><strong>Estándar: 8% a 12%</strong></p></>} />
                </div>
                <div className="relative">
          ...
          <div className="glass rounded-2xl p-8 border border-emerald/20 mt-8 fade-in bg-surface/30">
            ...
            <div className="grid md:grid-cols-2 gap-12 mb-12">
                ...
                <div className="space-y-2 mb-6">
                  {result.projectedFlows.map(f => (
                    <div key={f.year} className="flex justify-between text-sm font-mono border-b border-border/50 pb-1">
                      ...
                      <div className="text-right"><span className="text-text mr-4">{fmtPrice(f.fcf)}</span><span className="text-emerald/70 text-xs">PV: {fmtPrice(f.discounted)}</span></div>
                    </div>
                  ))}
                  <div className="flex justify-between text-sm font-mono font-bold pt-2"><span className="text-text">Suma PV Flujos</span><span className="text-emerald">{fmtPrice(result.pvSum)}</span></div>
                </div>

                <h4 className="text-xs uppercase tracking-widest text-muted mb-4 font-mono">2. De la Empresa a la Acción</h4>
                <div className="space-y-2 text-sm font-mono">
                  <div className="flex justify-between"><span className="text-subtle">Suma PV Flujos</span><span className="text-text">{fmtPrice(result.pvSum)}</span></div>
                  <div className="flex justify-between"><span className="text-subtle">+ Valor Terminal (PV)</span><span className="text-text">{fmtPrice(result.discountedTv)}</span></div>
                  <div className="flex justify-between font-bold border-t border-border/50 pt-1"><span className="text-text">Enterprise Value</span><span className="text-sky">{fmtPrice(result.enterpriseValue)}</span></div>
                  <div className="flex justify-between"><span className="text-subtle">- Deuda Neta</span><span className="text-text">{fmtPrice(netDebt)}</span></div>
                  <div className="flex justify-between font-bold border-t border-border/50 pt-1"><span className="text-text">Equity Value</span><span className="text-gold">{fmtPrice(result.equityValue)}</span></div>
                  <div className="flex justify-between"><span className="text-subtle">/ Acciones en Circ.</span><span className="text-text">{fmt(shares)}</span></div>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center bg-bg/80 rounded-2xl p-8 border border-border shadow-2xl relative overflow-hidden">
                ...
                <div className="text-6xl font-mono font-bold text-text mb-4 z-10 drop-shadow-lg">{fmtPrice(result.intrinsicValue)}</div>
                <div className="w-full border-t border-border/50 my-4 z-10"></div>
                <div className="flex w-full justify-between items-center z-10">
                  <span className="text-sm text-subtle">Cotización Mercado ({data.currency})</span>
            ...
            <div className="border-t border-border/50 pt-8">
              ...
              <div className="overflow-x-auto pb-4">
                <table className="w-full max-w-2xl mx-auto border-collapse">
                  <thead>
                    <tr>
                      <th className="p-2 border border-border/50 bg-bg/50 text-xs text-muted font-display uppercase tracking-widest w-24">WACC ↓ <br/> Crec. →</th>
                      {result.sensitivityMatrix[0].row.map((col, idx) => (
                        <th key={idx} className={`p-2 border border-border/50 font-mono text-sm ${col.g === result.baseG ? 'bg-gold/10 text-gold font-bold' : 'bg-bg/50 text-subtle'}`}>{fmtPct(col.g)}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {result.sensitivityMatrix.map((rowItem, rIdx) => (
                      <tr key={rIdx}>
                        <th className={`p-2 border border-border/50 font-mono text-sm text-right pr-4 ${rowItem.wacc === result.baseR ? 'bg-gold/10 text-gold font-bold' : 'bg-bg/50 text-subtle'}`}>{fmtPct(rowItem.wacc)}</th>
                        {rowItem.row.map((cell, cIdx) => {
                          if (cell.val === null) return <td key={cIdx} className="p-2 border border-border/50 text-center text-xs text-muted font-mono bg-surface">N/A</td>
                          const isBase = rowItem.wacc === result.baseR && cell.g === result.baseG
                          const margin = ((cell.val - result.currentPrice) / result.currentPrice)
                          let cellClass = "bg-surface text-text"
                          ...
                          return <td key={cIdx} className={`p-2 border border-border/50 text-center font-mono text-sm transition-colors hover:opacity-80 ${cellClass}`}>{fmtPrice(cell.val)}</td>
                        })}
                      </tr>
                    ))}
                  </tbody>
  ...
  return (
    <div className="fade-in max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center mb-6">
        <div>
          <h1 className="font-display font-bold text-2xl text-text flex items-center gap-2"><Calculator className="text-gold" />Laboratorio DCF</h1>
        </div>
        <SearchBar onSelect={item => navigate(`/dcf/${item.symbol}`)} placeholder="Buscar empresa para valuar..." className="w-full md:w-80" />
      </div>
      {!ticker && !loading && !data && (
        <div className="glass rounded-2xl p-16 text-center mt-8 border border-border/50 border-dashed">
          ...
          <p className="text-subtle max-w-sm mx-auto">Buscá una empresa para generar un modelo de valuación.</p>
        </div>
      )}
